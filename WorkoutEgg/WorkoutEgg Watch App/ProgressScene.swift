...
class ProgressScene: SKScene {
    ...
    func setPetData(_ petData: PetData?) {
        ...
        if let newStage = petData?.stage, newStage != previousStage {
            DebugConfig.debugPrint(
                "🔄 Pet stage changed from \(previousStage?.displayName ?? "nil") to \(newStage.displayName)"
            )
            ...
            let displayCalories =
                newStage == .egg
                ? Int(petData?.cumulativeCalories ?? 0) : Int(petData?.currentDayCalories ?? 0)
    ...
    private func setupProgressBar() {
        ...
        DebugConfig.debugPrint(
            "🎯 Progress bar positioned at y: \(progressBarY) for stage: \(petData?.stage.displayName ?? "unknown")"
        )
    }
    ...
    private func setupCaloriesLabel() {
        ...
        DebugConfig.debugPrint(
            "🏷️ Labels positioned - Calories: y=\(caloriesLabelY), Stage: y=\(stageLabelY) for stage: \(petData?.stage.displayName ?? "unknown")"
        )
    }
    ...
    private func setupFoodRendered(currentCalories: Int, currentFoodCount: Int) {
        ...
        for (index, stage) in foodStagesToRender.enumerated() {
            ...
            if foodTexture.size() != CGSize.zero {
                DebugConfig.debugPrint(
                    "✅ Successfully loaded \(foodTextureName), size: \(foodTexture.size())")
                ...
                DebugConfig.debugPrint(
                    "🍎 Positioned food stage \(stage) at: \(finalPosition), scale: 2")
                ...
                DebugConfig.debugPrint(
                    "❌ Failed to load food texture: \(foodTextureName) - texture size is zero")
            }
        }

        DebugConfig.debugPrint(
            "🍎 Food rendering complete. Total food sprites: \(foodSprites.count)")
        DebugConfig.debugPrint(
            "🍎 Food sprites in scene: \(foodSprites.map { $0.name ?? "unnamed" })")
    }
    ...
    func updateProgress(current: Int) {
        ...
        let currentFoodCount = petData?.getCurrentDayFeedCount() ?? 0
        DebugConfig.debugPrint(
            "📊 About to call setupFoodRendered with calories: \(currentCalories), feedCount: \(currentFoodCount)"
        )
        setupFoodRendered(currentCalories: currentCalories, currentFoodCount: currentFoodCount)
    }
    ...
    private func handleFoodTap(_ foodSprite: SKSpriteNode, at location: CGPoint) {
        ...
        // Remove the food sprite
        foodSprite.removeFromParent()
        if let index = foodSprites.firstIndex(of: foodSprite) {
            foodSprites.remove(at: index)
        }

        // Debug
        DebugConfig.debugPrint("🍽️ Fed pet! Remaining food: \(foodSprites.count)")

        // Update the display after feeding
        let displayCalories =
            petData.stage == .egg
            ? Int(petData.cumulativeCalories) : Int(petData.currentDayCalories)
        updateProgress(current: displayCalories)
